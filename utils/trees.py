...
class TreeParser:
    ...
    nodes: dict with node name str as key and anytree Node instance as value which contains all the nodes of the tree tree
...
"""
    ...
    def __str__(self):
        ...
        Returns
        -------
        msg: a str containing the output tree visualization
        """
        msg = """"""
        root = self.nodes[self.root_key]
    ...
    def read_from_json(self, fname):
        """
        Read the tree from a JSON file given as input
        ...
        Parameters
        ----------
        fname: str with the name of the file containing the tree
        """
    ...
    def get_labelInfo_byLevel(self, mode = "name"):
        root = self.nodes[self.root_key]
        if mode == "name":
            l = [[node.name for node in children] for children in LevelOrderGroupIter(root, maxlevel = 3)]
        elif mode == "code":
            l = [[node.code for node in children] for children in LevelOrderGroupIter(root, maxlevel = 3)]
        else:
            l = [[node.name for node in children] for children in LevelOrderGroupIter(root, maxlevel = 3)]

        return l

    def get_labelInfo_atNode(self, label, mode = "name"):
        nodeInTree = self.find_by_name(label)
        if mode == "name":
            l = [[node.name for node in children] for children in LevelOrderGroupIter(nodeInTree, maxlevel = 2)]
        elif mode == "code":
            l = [[node.code for node in children] for children in LevelOrderGroupIter(nodeInTree, maxlevel = 2)]
        else:
            l = [[node.name for node in children] for children in LevelOrderGroupIter(nodeInTree, maxlevel = 2)]
